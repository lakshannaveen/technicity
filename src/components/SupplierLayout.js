...
import { FiBox, FiFileText, FiGrid, FiShoppingCart, FiTool } from 'react-icons/fi';
...
const SupplierLayout = ({ children }) => {
  ...
  const navigation = [
    { name: 'Dashboard', href: '/supplier/dashboard', icon: FiGrid },
    { name: 'Orders', href: '/supplier/orders', icon: FiShoppingCart },
    { name: 'Inventory', href: '/supplier/inventory', icon: FiBox },
    { name: 'Supplier Receipts', href: '/supplier/receipts', icon: FiFileText },
  ];
          ...
          <div className="flex-shrink-0 flex border-t border-slate-200 p-3">
            <div className={`flex items-center w-full ${sidebarCollapsed ? 'justify-center' : ''}`}>
              <div className={`${sidebarCollapsed ? '' : 'ml-3'}`}>
                ...
                {!sidebarCollapsed && <p className="text-xs font-medium text-slate-500">Supplier</p>}
              </div>
              {!sidebarCollapsed ? (
                <button 
                ...
                <button onClick={handleLogout} className="ml-auto p-2 rounded-md text-slate-600 hover:bg-slate-100" title="Logout">
